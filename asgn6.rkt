...
; We fully implemented assignment 6
...
(define (sub-serialize [val : Value]) : String
  (match val
    [(NumV n) (format "~v" n)]
    [(BoolV b) (cond [b "true"]
                     [else "false"])]
...
;; println
;; println is given a string then prints that string to stdout, followed by a newline
(define (println [s : String]) : BoolV
  (cond
    [(void? (display (string-append s "\n"))) (BoolV #t)]))
...
(check-equal? (parse '{println}) (AppC (IdC 'println) '()))
